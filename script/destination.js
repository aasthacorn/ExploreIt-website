...
// This function creates individual cards
function createCard(item) {
  ...
  price.innerHTML = `<span>Rs.${item.estimatedCost}00</span> per person`;
...
// Filter prices: low to high or high to low
...
// Function to adjust prices for Indian cities
function adjustPriceForIndia(price, country) {
  if (country === "India") {
    return price / 10; // Divide the price by 10 if it's in India (i.e., lakhs to thousands)
  }
  return price; // For other countries, return the price as it is
}

// This function creates pagination buttons exclusively for filter By Country
...
// This function creates pagination buttons exclusively for filter by rating
...
// This function filters data by price option
...
// This function creates pagination buttons exclusively for filter by price
...
/******* Functionality of Destination Modal **********/

function createDetails(city) {
  let detailedObj = {}; // obj containing city attributes, foods and activities URLs
...
function displayDestinationDetails(imgObj, city) {
  ...
  let adjustedPrice = adjustPriceForIndia(city.estimatedCost, city.country);

  let price = document.createElement("p");
  price.innerHTML = `It would take about <span>Rs.${city.estimatedCost}00</span> per person to travel to ${city.name}`;
